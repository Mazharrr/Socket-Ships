...
class Player {
  ...
  update(game) {
    ...
    for (var i in players) {
      sortedPlayers[index] = {
        exp: players[i].sprite.exp,
        name: players[i].enemy.id
      };
      index++;
    }
    ...
    if (sortedPlayers[0])
      game.debug.text(
        "1." + sortedPlayers[0].name + "  Exp: " + sortedPlayers[0].exp,
        950,
        80
      );
    ...
    if (sortedPlayers[1])
      game.debug.text(
        "2." + sortedPlayers[1].name + "  Exp: " + sortedPlayers[1].exp,
        950,
        100
      );
    ...
    if (sortedPlayers[2])
      game.debug.text(
        "3." + sortedPlayers[2].name + "  Exp: " + sortedPlayers[2].exp,
        950,
        120
      );
    ...
    game.debug.text(
      this.sprite.exp + " EXP",
      this.sprite.x - game.camera.x - 15,
  ...
  this.socket.on("connect", () => {
    ...
    this.socket.on("claim_Chest", data => {
      ...
      if (this.player.id === data.id) {
        if (this.player.sprite.health > 50) this.player.sprite.health = 100;
        else {
...
function checkOverlap(bullets, ship) {
  ...
  for (var x = 0; x < bullets.children.length; x++) {
    ...
    if (
      Phaser.Rectangle.intersects(shipBounds, bullet) &&
      shipName != bullets.children[x].playerId
    )
      return x;
  }
  ...
  if (player) {
    ...
    if (Object.keys(players.length)) {
      for (var i in players) {
        ...
        players[i].healthBar.setPosition(
          players[i].sprite.x + 10,
          players[i].sprite.y - 100
        );
        bulletHit = null;
